...
import { setFavourite } from '../features/tracks/trackSlice'
...
export default function NewCharts({info}) {
    ...
    const handleFavourite = (e) => {
        ...
        setFav(!fav)
        dispatch(setFavourite(info))
    }

  return (
    <div className="charts" onClick={() => {navigate("/album")}}>
        <div className="chart-img">
            <img src={"../../assets/images/" + info.album_img} alt="" />
        </div>
        <div className="chart-text-ctn">
            <p className="chart-song-name">{info.name}</p>
            <p className="chart-artiste">{info.artiste || info.desc}</p>
            <p className="chart-duration">{info.duration || '--'}</p>
        </div>
        <div className="chart-fav-ctn" onClick={(e) => handleFavourite(e)}>
        <svg width="18" height="18" viewBox="0 0 18 18" fill={info.favorite ? '#E5524A': 'none'} xmlns="http://www.w3.org/2000/svg">
